...
class BenchmarkFoldingEnv(reach_env.ReachEnv):
  ...
  CENTER_GRAB_POSE = np.array([0.0, -0.6, -0.24, -0.308, -2.943, -0.035])
  ...
  TIMEOUT_PER_INSTRUCTION_SECONDS = 175000. # 600.0 TODO change this back if not doing data collection.
  ...
  def scramble(self, num_times=None) -> core.Observation:
    ...
    num_times = num_times or self.TSHIRT_TOSSING_CYCLES

    for i in range(1, num_times + 1):
      # Toss t-shirt toward the center of the table
      _log(f"Tossing t-shirt {i} of {num_times} times")
  ...
  def step(self,
    ...
    if "arm" in action:
      if "joint_angles" in action["arm"] and action["arm"]["synchronous"] != 1: 
        q_action = action["arm"]["joint_angles"]
        assert q_action is not None
        assert self.last_joint_move_observation is not None
